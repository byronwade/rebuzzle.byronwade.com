...
export function initializeSession(userId?: string): SessionData {
  ...
  if (lastVisit) {
    const timeSinceLastVisit = now - Number.parseInt(lastVisit, 10);
  ...
  return {
    sessionId,
    startTime: isNewSession
      ? now
      : Number.parseInt(sessionStart || now.toString(), 10),
    isReturningUser,
    userId,
  };
}
...
export function updateSessionUserId(userId: string): void {
  ...
  const sessionData = initializeSession(userId);
