 ...
 */
function calculatePointsEarned(gameResult: { won: boolean; attempts: number }): number {
  if (!gameResult.won) {
    return 0
  }
  ...
  return Math.max(BASE_POINTS - (gameResult.attempts - 1) * POINTS_PER_ATTEMPT, MIN_POINTS)
}

function calculateStreak(currentStats: UserStats | null, gameResult: { won: boolean }, isNewDay: boolean): number {
  if (!gameResult.won) {
    return 0
  }
...
function calculateDailyStreak(currentStats: UserStats | null, gameResult: { won: boolean }, isNewDay: boolean): number {
  if (!gameResult.won) {
...
function calculateNewStats(
  ...
  const lastPlayDate = currentStats?.lastPlayDate ? 
    new Date(currentStats.lastPlayDate.getFullYear(), currentStats.lastPlayDate.getMonth(), currentStats.lastPlayDate.getDate()) : 
    null
...
export async function getLeaderboard(limit = 10): Promise<Array<{
  ...
  try {
    const leaderboardData = await db.userStatsOps.getLeaderboard(limit)
...
export async function createOrUpdateUser(userData: {
  ...
  try {
    ...
    if (existingUser) {
      ...
      await db.userOps.update(userData.id, {
      ...
      await db.userOps.create({
        ...
        username: userData.username,
        email: userData.email,
        passwordHash: '', // Will be set by auth provider
        createdAt: new Date(),
        lastLogin: new Date(),
      })
    }
