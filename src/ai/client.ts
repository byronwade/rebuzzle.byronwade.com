...
class AIProvider {
  ...
  constructor() {
    ...
    switch (AI_CONFIG.defaultProvider) {
      case "google": {
        ...
        console.log(
          "[AI] Using Google AI (Gemini) directly - requires GOOGLE_AI_API_KEY"
        );
        console.log(
          "[AI] ⚠️ Consider using gateway provider with only AI_GATEWAY_API_KEY instead"
        );
        break;
      }
      case "groq":
      ...
      case "gateway": {
        ...
        if (gatewayApiKey) {
          ...
          console.log(
            `[AI] Gateway API key: ${keyPreview} (${gatewayApiKey.length} chars)`
          );
          // Validate key format (should start with vck_)
          if (!gatewayApiKey.startsWith("vck_")) {
            console.warn(
              `[AI] ⚠️ Gateway API key doesn't start with 'vck_' - might be invalid`
            );
          }
        } else {
          console.warn(
          ...
          console.warn(
            "[AI] ⚠️ If this fails, set AI_GATEWAY_API_KEY in .env.local"
          );
        }
        try {
          ...
          console.error(
            "[AI] ❌ Failed to initialize Gateway:",
            gatewayInitError
          );
          throw new Error(
      ...
      default:
        throw new Error(
          `Unsupported AI provider: ${AI_CONFIG.defaultProvider}`
        );
    }
  }
  ...
  getModel(type: "fast" | "smart" | "creative" = "smart"): string {
    ...
    const models =
      AI_CONFIG.models[this.providerName as keyof typeof AI_CONFIG.models];
    ...
    // Fallback
    throw new Error(
      `Model type "${type}" not found for provider "${this.providerName}"`
    );
  }
  ...
  for (let i = 0; i < modelsToTry.length; i++) {
    ...
    try {
      ...
      if (provider.getName() === "gateway") {
        ...
        const gatewayProvider = provider.getProvider() as ReturnType<
        ...
        try {
          ...
          if (process.env.NODE_ENV === "development") {
            console.error(
              `[AI] Gateway model creation failed for ${modelName}:`,
              errorMsg
            );
            console.error(
              `[AI] Gateway API key present: ${!!AI_CONFIG.gateway.apiKey}`
            );
            console.error(
        ...
        const groqProvider = provider.getProvider() as ReturnType<
          typeof createGroq
        >;
        modelInstance = groqProvider(modelName);
      } else {
        const xaiProvider = provider.getProvider() as ReturnType<
          typeof createXai
        >;
      ...
      const result = await generateText({
        ...
        temperature:
          params.temperature ?? AI_CONFIG.generation.temperature.balanced,
      ...
      const isQuotaError =
        ...
        parsedError instanceof QuotaExceededError;
      ...
      const isRetryable =
        ...
        (parsedError instanceof AIProviderError &&
      ...
      if (isRetryable && !isLastModel) {
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          console.warn(
            `[AI] Trying next fallback model (${i + 2}/${modelsToTry.length})...`
          );
        }
      } else {
...
export async function generateAIObject<T>(params: {
  ...
  for (let i = 0; i < modelsToTry.length; i++) {
    ...
    try {
      if (process.env.NODE_ENV === "development") {
        console.log(
          `[AI] Attempting model ${i + 1}/${modelsToTry.length}: ${modelName}`
        );
      }
          ...
          const errorDetails =
            generateError instanceof Error
              ? {
                  ...
                  ...((generateError as any).cause
                    ? { cause: (generateError as any).cause }
                    : {}),
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `[AI] ✓ Generated object with ${modelName} in ${duration}ms`,
          {
            provider: provider.getName(),
            modelType,
            usage: result.usage,
      ...
      if (attempt < maxAttempts) {
        const delay = Math.min(
          AI_CONFIG.retry.initialDelay *
            AI_CONFIG.retry.backoffMultiplier ** (attempt - 1),
          AI_CONFIG.retry.maxDelay
        );

        console.warn(
          `[AI] Attempt ${attempt} failed, retrying in ${delay}ms...`
        );
