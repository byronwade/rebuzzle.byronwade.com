...
class QuotaManager {
   ...
   */
  async checkQuota(): Promise<{ allowed: boolean; reason?: string; resetAt?: Date }> {
    this.resetIfNeeded()
   ...
   */
  shouldWarn(): { warn: boolean; level: "warning" | "critical" | "ok"; message: string } {
...
export async function enforceQuota(): Promise<void> {
  ...
  if (warning.warn && process.env.NODE_ENV === "development") {
    console.warn(`[AI Quota] ${warning.level.toUpperCase()}: ${warning.message}`)
  }
}
