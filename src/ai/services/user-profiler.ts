 ...
 */
export async function buildUserPuzzleProfile(
  userId: string
): Promise<UserPuzzleProfile> {
  // Get user stats
...
export async function calculateUserDifficultyPreference(
  ...
  const puzzles = await puzzleCollection
 ...
 */
export async function identifyUserCategories(
  userId: string
): Promise<string[]> {
  const attempts = await puzzleAttemptOps.getUserAttempts(userId, 100);
  const puzzleCollection = getCollection<Puzzle>("puzzles");
 ...
 */
export async function identifyUserPuzzleTypes(
  userId: string
): Promise<string[]> {
  const attempts = await puzzleAttemptOps.getUserAttempts(userId, 100);
  const puzzleCollection = getCollection<Puzzle>("puzzles");
