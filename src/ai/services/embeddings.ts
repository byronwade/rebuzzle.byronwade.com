...
function getEmbeddingModel() {
  ...
  if (providerName === "gateway") {
    ...
    const embeddingModelId =
      AI_CONFIG.embeddings.gateway || "google/gemini-embedding-001";
    ...
    try {
      ...
      const fallbackChain = AI_CONFIG.embeddings.fallbackChain || [
        embeddingModelId,
      ];
      ...
      for (const fallbackModelId of fallbackChain) {
        try {
          ...
          console.warn(
            `[Embeddings] Fallback model ${fallbackModelId} failed, trying next...`
          );
        }
      }
      ...
      const embeddingModel =
        AI_CONFIG.embeddings.google || "gemini-embedding-001";
