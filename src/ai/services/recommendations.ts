...
export async function recommendNextPuzzle(
  ...
  if (similar.length === 0) {
    ...
    const difficultyRecs = await getDifficultyBasedRecommendations(
      userId,
      profile,
      1
    );
...
export async function getAdaptiveDifficulty(userId: string): Promise<{
  ...
  if (recentAttempts.length < 3) {
    ...
    return {
      recommended:
        (profile.preferredDifficultyRange.min +
          profile.preferredDifficultyRange.max) /
        2,
  ...
  if (recentSolveRate > 0.8) {
    ...
    reasoning =
      "You're solving puzzles quickly - try something more challenging!";
