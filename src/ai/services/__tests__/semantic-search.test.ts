...
>;
const mockGeneratePuzzleEmbedding =
  generatePuzzleEmbedding as jest.MockedFunction<
    typeof generatePuzzleEmbedding
  >;
const mockCosineSimilarity = cosineSimilarity as jest.MockedFunction<
  typeof cosineSimilarity
>;
...
describe("Semantic Search Service", () => {
  ...
  describe("findSimilarPuzzles", () => {
    ...
    it("should generate embedding if not present", async () => {
      ...
      mockPuzzleOps.findById = jest
        .fn()
        .mockResolvedValue(puzzleWithoutEmbedding);
    ...
    it("should throw error if puzzle not found", async () => {
      ...
      await expect(findSimilarPuzzles("nonexistent")).rejects.toThrow(
        "Puzzle not found"
      );
    });
  ...
  describe("recommendPuzzlesByUserHistory", () => {
    it("should recommend puzzles based on user history", async () => {
      ...
      (puzzleAttemptOps.getUserAttempts as jest.Mock) = jest
        .fn()
        .mockResolvedValue(mockAttempts);
      (puzzleOps.findActivePuzzles as jest.Mock) = jest
        .fn()
        .mockResolvedValue(mockSolvedPuzzles);
      (puzzleOps.findById as jest.Mock) = jest
        .fn()
        .mockResolvedValue(mockSolvedPuzzles[0]);

      const result = await recommendPuzzlesByUserHistory("user-1", 5);

      expect(result.length).toBeGreaterThanOrEqual(0);
      expect(puzzleAttemptOps.getUserAttempts).toHaveBeenCalledWith(
        "user-1",
        50
      );
    });
