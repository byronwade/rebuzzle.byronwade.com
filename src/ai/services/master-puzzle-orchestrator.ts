...
export async function generateMasterPuzzle(
  ...
  if (featureFlags.learning && params.useLearningFeedback !== false) {
    try {
      ...
      console.warn(
        "[Master Generator] Learning feedback failed, using default difficulty:",
        error
      );
    }
  }
  ...
  if (featureFlags.recommendations && params.userId) {
    try {
      ...
      console.warn(
        "[Master Generator] Personalization failed, using default difficulty:",
        error
      );
    }
  }
  ...
  const getQualityThreshold = (attempt: number) => {
    if (attempt === 1)
      return Math.max(
        GLOBAL_CONTEXT.qualityStandards.revisionThreshold,
        baseQualityThreshold - 10
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      console.log(
        `[Master Generator] Attempt ${attempt}/${maxAttempts} (type: ${puzzleType})`
      );
      ...
      if (featureFlags.agentOrchestration) {
        try {
          ...
          calibratedDifficulty =
            orchestrationResult.metadata?.calibratedDifficulty ||
            candidatePuzzle.difficulty;
          difficultyProfile =
            orchestrationResult.metadata?.difficultyProfile || {};
          orchestrationMetadata = orchestrationResult.metadata;

          // Handle uniqueness failures by retrying
          if (!orchestrationResult.isUnique) {
            console.log(
              "[Stage 1] Puzzle failed uniqueness check in orchestration, retrying..."
            );
            continue; // Try again
          }
          ...
          if (puzzleType === "rebus") {
            ...
            const puzzleText =
              puzzleAny[puzzleDisplayField] || puzzleAny.puzzle || "";
            if (puzzleText) {
          ...
          const orchestrationQualityMetrics =
            orchestrationResult.metadata?.qualityMetrics;
          const orchestrationVerdict =
            orchestrationQualityMetrics?.analysis?.verdict;
          const isPublishVerdict =
            orchestrationVerdict === "excellent" ||
            orchestrationVerdict === "good";
      ...
      if (!(featureFlags.agentOrchestration && candidatePuzzle)) {
        ...
        if (config.validation.validate) {
          ...
          if (!validation.valid) {
            console.log(
              `[Stage 1] Validation failed: ${validation.errors.join(", ")}`
            );
            continue; // Try again
          }
        }
        ...
        if (puzzleType === "rebus" && candidatePuzzle.rebusPuzzle) {
          ...
          if (
            ...
            uniquenessCheck.similarityScore > 0.8
          ) {
      ...
      const result: GeneratedPuzzleResult = {
        ...
        metadata: {
          fingerprint,
          uniquenessScore:
            orchestrationMetadata?.uniquenessScore || uniquenessScore,
          difficultyProfile:
            orchestrationMetadata?.difficultyProfile || difficultyProfile,
          calibratedDifficulty:
            orchestrationMetadata?.calibratedDifficulty || calibratedDifficulty,
          qualityMetrics:
            orchestrationMetadata?.qualityMetrics ||
      ...
      const meetsThreshold =
        qualityResults.finalScore >= currentThreshold && qualityResults.passed;
      ...
      const robustness =
        qualityResults.adversarialResults?.overallRobustness ?? "skipped";
      console.log(
  ...
  throw new Error(
    `Failed to generate acceptable puzzle after ${maxAttempts} attempts`
  );
}
...
export async function selectOptimalPuzzle(params: {
  ...
  if (params.recentDifficulties && params.recentDifficulties.length >= 3) {
    const recentAvg =
      params.recentDifficulties.reduce((a, b) => a + b, 0) /
      params.recentDifficulties.length;
