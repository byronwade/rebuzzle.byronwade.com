...
export function calculateDifficultyProfile(puzzle: {
  ...
  const cognitiveSteps = Math.min(
    10,
    stepIndicators.filter((indicator) =>
  ...
  const isIdiom =
    puzzle.category.includes("idiom") || puzzle.category.includes("phrase");
...
export function calculateAdaptiveDifficulty(params: {
  ...
  const avgSuccess =
    params.recentPerformance.length > 0
      ? params.recentPerformance.filter((p) => p.solved).length /
        params.recentPerformance.length
      : 0.5;
