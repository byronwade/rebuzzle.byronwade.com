...
const CATEGORY_COMPLEXITY_ANAGRAM = 3;
const CATEGORY_COMPLEXITY_WORD_SEARCH = 4;
const CATEGORY_COMPLEXITY_CROSSWORD = 6;
const CATEGORY_COMPLEXITY_WORD_LADDER = 7;
const CATEGORY_COMPLEXITY_CRYPTOGRAM = 9;
const CATEGORY_COMPLEXITY_DEFAULT = 5;

// Word puzzle schema
export const WordPuzzleSchema = z.object({
  puzzle: z
    .string()
    .describe("The word puzzle (e.g., anagram, word search clue, etc.)"),
...
export const WORD_PUZZLE_CONFIG: PuzzleTypeConfig = {
  ...
  name: "Word Puzzle",
  description:
    "Various word-based puzzles including anagrams, word searches, and more",
    ...
    calculate: (puzzle: unknown) => {
      ...
      if (!scores) {
        ...
        const baseDifficulty =
          typedPuzzle.answer.length / ANSWER_LENGTH_DIVISOR;
        ...
        return Math.round(
          Math.max(
            DIFFICULTY_MIN,
            Math.min(DIFFICULTY_MAX, baseDifficulty * categoryMultiplier)
          )
        );
      }
    ...
    factors: [
      {
        ...
        extract: (puzzle: unknown) =>
          ...
          Math.min(
            10,
            (puzzle as WordPuzzle).answer.length / ANSWER_LENGTH_EXTRACT_DIVISOR
          ),
      },
      {
      ...
      },
      {
        ...
        weight: 0.2,
        extract: (puzzle: unknown) =>
          (puzzle as WordPuzzle).complexityScore?.vocabularyLevel || 5,
      },
      {
        name: "patternObscurity",
        weight: 0.2,
        extract: (puzzle: unknown) =>
          (puzzle as WordPuzzle).complexityScore?.patternObscurity || 5,
      },
      {
        name: "cognitiveSteps",
        weight: 0.15,
        extract: (puzzle: unknown) =>
          (puzzle as WordPuzzle).complexityScore?.cognitiveSteps || 5,
      },
    ],
  },
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const difficultyScore =
            (DIFFICULTY_INVERT_BASE - typedPuzzle.difficulty) *
            DIFFICULTY_INVERT_MULTIPLIER;
