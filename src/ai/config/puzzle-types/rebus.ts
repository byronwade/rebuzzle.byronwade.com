...
Show your thinking process, then create the puzzle with CORRECT INTEGER complexity scores (1-10, not decimals).`;
  ...
  difficulty: {
    calculate: (puzzle: unknown) => {
      ...
      const score =
        typedPuzzle.complexityScore.visualAmbiguity *
          (weights.visualAmbiguity ?? 0.2) +
        typedPuzzle.complexityScore.cognitiveSteps *
          (weights.cognitiveSteps ?? 0.3) +
        typedPuzzle.complexityScore.culturalKnowledge *
          (weights.culturalKnowledge ?? 0.2) +
        typedPuzzle.complexityScore.vocabularyLevel *
          (weights.vocabularyLevel ?? 0.15) +
    ...
    factors: [
      {
        ...
        extract: (puzzle: unknown) =>
          (puzzle as RebusPuzzle).complexityScore.visualAmbiguity,
      },
      {
        name: "cognitiveSteps",
        weight: 0.3,
        extract: (puzzle: unknown) =>
          (puzzle as RebusPuzzle).complexityScore.cognitiveSteps,
      },
      {
        name: "culturalKnowledge",
        weight: 0.2,
        extract: (puzzle: unknown) =>
          (puzzle as RebusPuzzle).complexityScore.culturalKnowledge,
      },
      {
        name: "vocabularyLevel",
        weight: 0.15,
        extract: (puzzle: unknown) =>
          (puzzle as RebusPuzzle).complexityScore.vocabularyLevel,
      },
      {
        name: "patternNovelty",
        weight: 0.15,
        extract: (puzzle: unknown) =>
          (puzzle as RebusPuzzle).complexityScore.patternNovelty,
      },
    ],
  },
    ...
    dimensions: [
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          const typedPuzzle = puzzle as RebusPuzzle;
          const emojiCount = (
            typedPuzzle.rebusPuzzle.match(/[\p{Emoji}]/gu) || []
          ).length;
          return Math.min(
            QUALITY_SCORE_MAX,
            emojiCount * EMOJI_COUNT_MULTIPLIER
          );
        },
      },
      {
