...
export function parseAIError(error: unknown): AIError {
  ...
  if (
    ...
    if (errorObj) {
      ...
      if (
        ...
        err.data?.error?.type === "model_not_found"
      ) {
  ...
  if (errorObj) {
    ...
    if (err.lastError) {
      ...
      if (
        nested instanceof QuotaExceededError ||
        nested.code === "QUOTA_EXCEEDED"
      ) {
        return nested;
      }
    ...
    if (
      ...
      return new AIError(
        gatewayErr.data?.error?.message || errorMessage,
        "MODEL_NOT_FOUND",
        404
      );
    }
    ...
    if (err.status === 429 || err.statusCode === 429 || err.code === "429") {
      const resetTime = err.headers?.["x-ratelimit-reset"]
        ? new Date(Number.parseInt(err.headers["x-ratelimit-reset"]) * 1000)
        : undefined;
