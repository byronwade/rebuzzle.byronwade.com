...
export function BulkBlogPostGenerator({
  onBlogPostsSaved,
}: BulkBlogPostGeneratorProps) {
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [generatedPosts, setGeneratedPosts] = useState<any[]>([]);
  const [selectedPosts, setSelectedPosts] = useState<Set<number>>(new Set());
  const [mode, setMode] = useState<
    "without-blogs" | "date-range" | "puzzle-ids"
  >("without-blogs");
  ...
  const handleSaveSelected = async () => {
    const postsToSave = Array.from(selectedPosts).map(
      (index) => generatedPosts[index]
    );
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Generate blog posts for multiple puzzles at once
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={(value) =>
                  setMode(
                    value as "without-blogs" | "date-range" | "puzzle-ids"
                  )
                }
                value={mode}
              >
