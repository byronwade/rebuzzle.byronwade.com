...
async function generate() {
	...
	if (fs.existsSync(OUTPUT_FILE)) {
		const existingPuzzles: Puzzle[] = JSON.parse(fs.readFileSync(OUTPUT_FILE, "utf-8"));
		existingPuzzles.forEach((p: Puzzle) => existingKeywords.add(p.keyword.toLowerCase()));
		console.log(`Loaded ${existingKeywords.size} existing keywords to avoid duplicates.`);
		allPuzzles.push(...existingPuzzles);
	}

	while (allPuzzles.length < NUM_PUZZLES_TO_GENERATE) {
		console.log(`Generated ${allPuzzles.length} of ${NUM_PUZZLES_TO_GENERATE} puzzles.`);
		try {
			const puzzleData = await generateSinglePuzzleForOfflineUse(Array.from(existingKeywords));

			if (puzzleData && !existingKeywords.has(puzzleData.keyword.toLowerCase())) {
	...
	console.log(`Successfully generated puzzles. Total count: ${allPuzzles.length}. Saved to ${OUTPUT_FILE}`);
}
