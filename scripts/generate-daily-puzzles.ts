...
async function generatePuzzleForDate(date: Date): Promise<void> {
  console.log(
    `🧩 Generating puzzle for ${date.toISOString().split("T")[0]}...`
  );
  ...
  try {
    ...
    if (existingPuzzle) {
      console.log(
        `✅ Puzzle already exists for ${date.toISOString().split("T")[0]}`
      );
      return;
    }
    ...
    console.error(
      `❌ Error generating puzzle for ${date.toISOString().split("T")[0]}:`,
      error
    );
    throw error;
  }
...
async function generatePuzzlesForDates(dates: Date[]): Promise<void> {
  ...
  for (const date of dates) {
    try {
      ...
      console.error(
        `❌ Failed to generate puzzle for ${date.toISOString().split("T")[0]}:`,
        error
      );
