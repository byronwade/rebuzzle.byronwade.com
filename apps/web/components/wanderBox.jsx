				...
				<div key={wordIndex} className="flex space-x-2 mb-4">
					{word.map((char, charIndex) =>
						isPunctuation(char) ? (
							...
							<Input key={charIndex} id={`input-${wordIndex}-${charIndex}`} type="text" maxLength={1} value={char} onChange={(event) => handleChange(event, wordIndex, charIndex)} onKeyDown={(event) => handleKeyDown(event, wordIndex, charIndex)} className={`md:w-12 md:h-12 w-10 h-10 text-center md:text-lg font-bold ${guessFeedback[wordIndex][charIndex]}`} autoComplete="off" disabled={gameOver} />
						)
					)}
				</div>
			))}
