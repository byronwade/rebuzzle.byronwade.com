...
export const LogicGridPuzzleSchema = z.object({
  ...
  explanation: z
    .string()
    .describe("Step-by-step explanation of how to solve the puzzle using deductive reasoning"),
  category: z
    .enum([
      "classic_einstein",
      "who_owns_what",
  ...
  categories: z
    ...
    .min(CATEGORIES_MIN)
    .max(CATEGORIES_MAX)
    .describe("The categories in the puzzle (e.g., ['People', 'Houses', 'Pets', 'Drinks'])"),
  items: z
  ...
  grid: z.string().optional().describe("Optional grid representation of the puzzle"),
});
  ...
  difficulty: {
    ...
    factors: [
      ...
      },
      {
        ...
        extract: (puzzle: unknown) => (puzzle as LogicGridPuzzle).complexityScore.numberOfItems,
      },
      {
        name: "clueComplexity",
        weight: WEIGHT_CLUE_COMPLEXITY,
        extract: (puzzle: unknown) => (puzzle as LogicGridPuzzle).complexityScore.clueComplexity,
      },
      {
        name: "deductiveSteps",
        weight: WEIGHT_DEDUCTIVE_STEPS,
        extract: (puzzle: unknown) => (puzzle as LogicGridPuzzle).complexityScore.deductiveSteps,
      },
      {
    ...
    dimensions: [
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const itemsDefined = Object.keys(typedPuzzle.items).length === categoriesCount;
          const sufficientClues = cluesCount >= categoriesCount * CLUES_PER_CATEGORY_MULTIPLIER;
          return itemsDefined && sufficientClues ? QUALITY_SCORE_VERY_HIGH : QUALITY_SCORE_MEDIUM;
        },
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          return hasExplanation && hasAnswer ? QUALITY_SCORE_HIGH : QUALITY_SCORE_MEDIUM;
        },
      },
      {
        name: "creativity",
        ...
        score: (puzzle: unknown) => {
          ...
          const complexity = typedPuzzle.complexityScore?.clueComplexity || COMPLEXITY_FALLBACK;
          return Math.min(QUALITY_SCORE_MAX, baseScore + complexity * COMPLEXITY_MULTIPLIER);
        },
      },
      {
