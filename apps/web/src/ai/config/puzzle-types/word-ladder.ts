...
export const WordLadderPuzzleSchema = z.object({
  puzzle: z
    .string()
    .describe("The word ladder puzzle description (e.g., 'Transform COLD into WARM in 4 steps')"),
  answer: z
    .string()
    .describe("The complete solution showing all steps (e.g., 'COLD → CORD → CARD → WARD → WARM')"),
  difficulty: z
    .number()
    .min(DIFFICULTY_MIN)
    .max(DIFFICULTY_MAX)
  ...
  category: z
    .enum(["same_length", "different_length", "themed", "minimum_steps", "maximum_steps"])
  ...
  ladder: z
    .array(z.string())
    .optional()
    .describe("The complete word ladder sequence (all words from start to end)"),
});
    ...
    factors: [
      {
        ...
        extract: (puzzle: unknown) => (puzzle as WordLadderPuzzle).complexityScore.pathObscurity,
      },
      {
        name: "stepsRequired",
        weight: WEIGHT_STEPS_REQUIRED,
        extract: (puzzle: unknown) => (puzzle as WordLadderPuzzle).complexityScore.stepsRequired,
      },
      {
        name: "wordLength",
        weight: WEIGHT_WORD_LENGTH,
        extract: (puzzle: unknown) => (puzzle as WordLadderPuzzle).complexityScore.wordLength,
      },
      {
        name: "vocabularyLevel",
        weight: WEIGHT_VOCABULARY_LEVEL,
        extract: (puzzle: unknown) => (puzzle as WordLadderPuzzle).complexityScore.vocabularyLevel,
      },
      {
        name: "wordSimilarity",
        weight: WEIGHT_WORD_SIMILARITY,
        extract: (puzzle: unknown) => (puzzle as WordLadderPuzzle).complexityScore.wordSimilarity,
      },
    ],
  },
    ...
    dimensions: [
      {
        ...
        score: (puzzle: unknown) => {
          ...
          return typedPuzzle.ladder && typedPuzzle.ladder.length === typedPuzzle.steps
            ? QUALITY_SCORE_HIGH
            : QUALITY_SCORE_FAIR;
        },
      },
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const validSteps = typedPuzzle.steps >= STEPS_MIN && typedPuzzle.steps <= STEPS_MAX;
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          return similarity >= SIMILARITY_SWEET_SPOT_MIN && similarity <= SIMILARITY_SWEET_SPOT_MAX
            ? QUALITY_SCORE_VERY_HIGH
            : QUALITY_SCORE_FAIR;
        },
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          return difficulty >= DIFFICULTY_SWEET_SPOT_MIN && difficulty <= DIFFICULTY_SWEET_SPOT_MAX
            ? QUALITY_SCORE_VERY_HIGH
            : QUALITY_SCORE_FAIR;
        },
      },
