...
export const NumberSequencePuzzleSchema = z.object({
  puzzle: z.string().describe("The number sequence puzzle (e.g., '2, 4, 8, 16, ?')"),
  answer: z.string().describe("The answer to the puzzle (the next number(s) in the sequence)"),
  difficulty: z
    .number()
    .min(DIFFICULTY_MIN)
    .max(DIFFICULTY_MAX)
  ...
  explanation: z
    .string()
    .describe("Clear explanation of the pattern and how to arrive at the answer"),
  category: z
    .enum([
      "arithmetic",
      "geometric",
  ...
  difficulty: {
    ...
    factors: [
      ...
      },
      {
        ...
        extract: (puzzle: unknown) =>
          (puzzle as NumberSequencePuzzle).complexityScore.mathematicalKnowledge,
      },
      {
        name: "numberRange",
        weight: WEIGHT_NUMBER_RANGE,
        extract: (puzzle: unknown) => (puzzle as NumberSequencePuzzle).complexityScore.numberRange,
      },
    ],
  },
  ...
  qualityMetrics: {
    dimensions: [
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          const typedPuzzle = puzzle as NumberSequencePuzzle;
          const novelCategories = ["recursive", "polynomial", "power_series", "mixed_pattern"];
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const hasEnoughNumbers = typedPuzzle.sequence.length >= SEQUENCE_MIN_LENGTH_FOR_QUALITY;
