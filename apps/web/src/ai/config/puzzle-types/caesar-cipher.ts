...
export const CaesarCipherPuzzleSchema = z.object({
  puzzle: z
    .string()
    .describe("The encrypted text (ciphertext) that needs to be decoded (e.g., 'IFMMP XPSME')"),
  ...
  explanation: z
    .string()
    .describe("Clear explanation of how the cipher works and how to decode the message"),
  category: z
    .enum(["rot13", "rot3", "custom_shift", "word_cipher", "phrase_cipher", "sentence_cipher"])
  ...
  shift: z.number().min(SHIFT_MIN).max(SHIFT_MAX).describe("The Caesar cipher shift amount (1-25)"),
  originalMessage: z
    .string()
    .optional()
...
export const CAESAR_CIPHER_CONFIG: PuzzleTypeConfig = {
  id: "caesar-cipher",
  name: "Caesar Cipher",
  description: "Decode encrypted text using letter shifting (ROT13, ROT3, etc.)",
...
Show your thinking process, then create the puzzle with CORRECT INTEGER complexity scores (1-10, not decimals).`;
  ...
  validation: {
    requiredFields: ["puzzle", "answer", "difficulty", "explanation", "category", "hints", "shift"],
    constraints: {
      puzzle: {
  ...
  difficulty: {
    ...
    factors: [
      ...
      },
      {
        ...
        extract: (puzzle: unknown) => (puzzle as CaesarCipherPuzzle).complexityScore.clueProvided,
      },
      {
        name: "messageLength",
        weight: WEIGHT_MESSAGE_LENGTH,
        extract: (puzzle: unknown) => (puzzle as CaesarCipherPuzzle).complexityScore.messageLength,
      },
      {
    ...
    dimensions: [
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const validShift = typedPuzzle.shift >= SHIFT_MIN && typedPuzzle.shift <= SHIFT_MAX;
