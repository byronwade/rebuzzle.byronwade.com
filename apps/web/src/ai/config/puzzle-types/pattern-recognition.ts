...
export const PatternRecognitionPuzzleSchema = z.object({
  puzzle: z.string().describe("The pattern sequence (e.g., '🔴 🔵 🔴 🔵 ?' or 'A, B, C, D, ?')"),
  answer: z.string().describe("The answer to complete the pattern (the missing element)"),
  difficulty: z
    .number()
    .min(DIFFICULTY_MIN)
    .max(DIFFICULTY_MAX)
  ...
  explanation: z
    .string()
    .describe("Clear explanation of the pattern and how to identify what comes next"),
  category: z
    .enum([
      "visual_sequence",
      "shape_pattern",
...
export type PatternRecognitionPuzzle = z.infer<typeof PatternRecognitionPuzzleSchema>;
    ...
    factors: [
      {
        ...
        extract: (puzzle: unknown) =>
          (puzzle as PatternRecognitionPuzzle).complexityScore.patternComplexity,
      },
      {
        name: "layersRequired",
    ...
    dimensions: [
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const hasEnoughElements = typedPuzzle.sequence.length >= SEQUENCE_MIN_LENGTH_FOR_QUALITY;
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          const typedPuzzle = puzzle as PatternRecognitionPuzzle;
          const novelCategories = ["mixed_pattern", "positional_pattern", "layersRequired"];
