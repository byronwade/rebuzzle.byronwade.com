...
export const CrypticCrosswordPuzzleSchema = z.object({
  puzzle: z
    .string()
    .describe("The cryptic crossword clue (e.g., 'Brave insect goes back and forth (4)')"),
  answer: z.string().describe("The answer to the cryptic clue (single word or phrase)"),
  difficulty: z
    .number()
    .min(DIFFICULTY_MIN)
    .max(DIFFICULTY_MAX)
  ...
  explanation: z
    .string()
    .describe("Clear explanation of how the cryptic clue works, breaking down the wordplay"),
  category: z
    .enum([
      "anagram_clue",
      "hidden_word",
  ...
  answerLength: z.number().optional().describe("Length of the answer (in letters or words)"),
});

export type CrypticCrosswordPuzzle = z.infer<typeof CrypticCrosswordPuzzleSchema>;
  ...
  difficulty: {
    ...
    factors: [
      ...
      },
      {
        ...
        extract: (puzzle: unknown) =>
          (puzzle as CrypticCrosswordPuzzle).complexityScore.linguisticKnowledge,
      },
      {
    ...
    dimensions: [
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const hasBreakdown =
            ...
            typedPuzzle.explanation.length > EXPLANATION_MIN_LENGTH_FOR_BREAKDOWN;
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          const typedPuzzle = puzzle as CrypticCrosswordPuzzle;
          const complexCategories = ["mixed", "container", "charade", "reversal"];
