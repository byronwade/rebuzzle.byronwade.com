...
export const RiddlePuzzleSchema = z.object({
  puzzle: z.string().describe("The riddle question or statement that needs to be solved"),
  answer: z.string().describe("The answer to the riddle (single word or phrase)"),
  difficulty: z
    .number()
    .min(4)
    .max(8)
  ...
  explanation: z
    .string()
    .describe("Clear explanation of the riddle's logic and how to arrive at the answer"),
  category: z
    .enum([
      "logic",
      "wordplay",
    ...
    factors: [
      {
        ...
        extract: (puzzle: unknown) => (puzzle as RiddlePuzzle).complexityScore.lateralThinking,
      },
      {
        name: "wordplayComplexity",
        weight: WEIGHT_WORDPLAY_COMPLEXITY,
        extract: (puzzle: unknown) => (puzzle as RiddlePuzzle).complexityScore.wordplayComplexity,
      },
      {
        name: "logicalSteps",
        weight: WEIGHT_LOGICAL_STEPS,
        extract: (puzzle: unknown) => (puzzle as RiddlePuzzle).complexityScore.logicalSteps,
      },
      {
        name: "culturalKnowledge",
        weight: WEIGHT_CULTURAL_KNOWLEDGE,
        extract: (puzzle: unknown) => (puzzle as RiddlePuzzle).complexityScore.culturalKnowledge,
      },
      {
        name: "vocabularyLevel",
        weight: WEIGHT_VOCABULARY_LEVEL,
        extract: (puzzle: unknown) => (puzzle as RiddlePuzzle).complexityScore.vocabularyLevel,
      },
    ],
  },
    ...
    dimensions: [
      {
        ...
        score: (puzzle: unknown) => {
          ...
          return hasQuestion && length > PUZZLE_MIN_LENGTH && length < PUZZLE_MAX_LENGTH
            ? QUALITY_SCORE_HIGH
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const noveltyCategories = ["lateral_thinking", "paradox", "double_meaning"];
          const baseScore = noveltyCategories.includes(typedPuzzle.category)
            ? BASE_SCORE_NOVEL
            : BASE_SCORE_STANDARD;
          const complexity = typedPuzzle.complexityScore?.wordplayComplexity || 5;
          return Math.min(QUALITY_SCORE_EXCELLENT, baseScore + complexity * COMPLEXITY_MULTIPLIER);
        },
      },
      {
        name: "solvability",
        ...
        score: (puzzle: unknown) => {
          ...
          const hasExplanation = typedPuzzle.explanation.length > EXPLANATION_MIN_LENGTH;
          const hasHints = typedPuzzle.hints.length >= HINTS_MIN_FOR_SOLVABILITY;
          return hasExplanation && hasHints ? QUALITY_SCORE_HIGH : QUALITY_SCORE_MEDIUM;
        },
      },
      {
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const interestingCategories = ["lateral_thinking", "wordplay", "paradox"];
          ...
          const difficultyScore =
            difficulty >= DIFFICULTY_SWEET_SPOT_MIN && difficulty <= DIFFICULTY_SWEET_SPOT_MAX
