...
export const TriviaPuzzleSchema = z.object({
  ...
  answer: z.string().describe("The answer to the trivia question (single word or phrase)"),
  difficulty: z
    .number()
    .min(DIFFICULTY_MIN)
    .max(DIFFICULTY_MAX)
  ...
  explanation: z
    .string()
    .describe("Clear explanation or context for the answer, including interesting facts"),
  category: z
    .enum([
      "geography",
      "history",
...
Show your thinking process, then create the trivia question with CORRECT INTEGER complexity scores (1-10, not decimals).`;
  ...
  validation: {
    requiredFields: ["puzzle", "answer", "difficulty", "explanation", "category", "hints"],
    constraints: {
      puzzle: {
        min: 10,
        max: 300,
    ...
    factors: [
      {
        ...
        extract: (puzzle: unknown) => (puzzle as TriviaPuzzle).complexityScore.knowledgeDepth,
      },
      {
        name: "topicObscurity",
        weight: WEIGHT_TOPIC_OBSCURITY,
        extract: (puzzle: unknown) => (puzzle as TriviaPuzzle).complexityScore.topicObscurity,
      },
      {
        name: "recallDifficulty",
        weight: WEIGHT_RECALL_DIFFICULTY,
        extract: (puzzle: unknown) => (puzzle as TriviaPuzzle).complexityScore.recallDifficulty,
      },
      {
        name: "specificity",
        weight: WEIGHT_SPECIFICITY,
        extract: (puzzle: unknown) => (puzzle as TriviaPuzzle).complexityScore.specificity,
      },
      {
        name: "culturalRelevance",
        weight: WEIGHT_CULTURAL_RELEVANCE,
        extract: (puzzle: unknown) => (puzzle as TriviaPuzzle).complexityScore.culturalRelevance,
      },
    ],
  },
      ...
      },
      {
        ...
        score: (puzzle: unknown) => {
          ...
          const hasExplanation =
            typedPuzzle.explanation.length > EXPLANATION_MIN_LENGTH_FOR_SOLVABILITY;
          const hasHints = typedPuzzle.hints.length >= HINTS_MIN_FOR_SOLVABILITY;
          return hasExplanation && hasHints ? QUALITY_SCORE_GOOD : QUALITY_SCORE_LOW;
        },
      },
      {
        name: "engagement",
        ...
        score: (puzzle: unknown) => {
          const typedPuzzle = puzzle as TriviaPuzzle;
          const interestingCategories = ["science", "history", "geography", "nature"];
