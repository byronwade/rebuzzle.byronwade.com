...
export function getFeatureFlags(): AIFeatureFlags {
  const flags: AIFeatureFlags = {
    ...
    embeddings:
      process.env.AI_ENABLE_EMBEDDINGS !== "false" && process.env.AI_ENABLE_EMBEDDINGS !== "0",

    // Learning: Enable learning from user feedback and performance analysis
    learning: process.env.AI_ENABLE_LEARNING !== "false" && process.env.AI_ENABLE_LEARNING !== "0",
