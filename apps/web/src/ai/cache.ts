...
class InMemoryCache {
  ...
  async getBySimilarity<T>(
    ...
    const operationEntries = this.semanticCache.filter((entry) => entry.key.includes(operation));
  ...
  set(key: string, value: unknown, ttlSeconds: number, embedding?: number[]): void {
...
export async function withCache<T>(
  ...
  if (options?.enableSemanticCache && options?.semanticCacheKey) {
    try {
      ...
      const semanticMatch = await cache.getBySimilarity<T>(queryEmbedding, operation);
...
export function getCacheStats(): {
  ...
  const cache = getCache();
  const semanticCache = (cache as any).semanticCache as CacheEntry[] | undefined;
  return {
    size: cache.size(),
