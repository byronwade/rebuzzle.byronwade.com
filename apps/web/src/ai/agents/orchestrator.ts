...
export async function orchestratePuzzleGeneration(params: {
  ...
  console.log("[Orchestrator] Agent 1: Generating puzzle with chain-of-thought...");
  const generationResult = await generateWithGeneratorAgent(modelInstance, params);
  ...
  if (puzzleType === "rebus") {
    ...
    if (puzzleText) {
      ...
      fingerprint = createPuzzleFingerprint({
        ...
        category: generationResult.puzzle.category || params.category || "general",
      });
  ...
  const qualityResult = await evaluateQualityWithAgent(modelInstance, generationResult.puzzle, {
...
export async function orchestratePersonalizedGeneration(
  ...
  try {
    ...
    const _puzzles = await Promise.all(puzzleIds.map((id) => puzzleOps.findById(id)));
  ...
  const personalizedResult = await generatePersonalizedWithAgent(modelInstance, {
    ...params,
    userContext,
  });
...
async function generateWithGeneratorAgent(
  _model: Parameters<typeof generateObject>[0]["model"] | any,
  params: {
    targetDifficulty: number;
    category?: string;
    theme?: string;
  ...
  return {
    puzzle: {
      ...
      ...(puzzleType === "rebus" && puzzleText ? { rebusPuzzle: puzzleText } : {}),
    } as Puzzle,
...
async function evaluateQualityWithAgent(
  _model: Parameters<typeof generateText>[0]["model"] | any,
  puzzle: Puzzle,
...
async function calibrateDifficultyWithAgent(
  ...
  if (!configCalculationSuccessful) {
    ...
    const puzzleTypeForCalibration = puzzleTypeToUse === "rebus" ? puzzleTypeToUse : "rebus";
    const puzzleDisplayField = puzzleTypeForCalibration === "rebus" ? "rebusPuzzle" : "puzzle";
    const puzzleAny = puzzle as any;
    const puzzleText = puzzleAny[puzzleDisplayField] || puzzleAny.puzzle || "";
    ...
    if (puzzleTypeForCalibration === "rebus" && puzzleText) {
      const calibration = await calibrateDifficulty({
        rebusPuzzle: puzzleText,
        answer: puzzle.answer,
        proposedDifficulty: typeof puzzle.difficulty === "number" ? puzzle.difficulty : 5,
        hints: puzzle.hints || [],
      });
      ...
      calibratedDifficulty =
        typeof puzzle.difficulty === "number" ? puzzle.difficulty : targetDifficulty;
      difficultyProfile = {};
      reasoning = "Using proposed difficulty (no calibration available for this puzzle type)";
      factors = [];
    }
  }
