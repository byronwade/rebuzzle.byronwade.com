...
class AIMonitor {
  ...
  trackOperation(event: Omit<AIEvent, "timestamp">): void {
    ...
    if (event.tokens) {
      ...
      // Calculate cost
      this.metrics.totalCost += this.calculateCost(event.provider, event.model, event.tokens);
    }
  ...
  private calculateCost(
    ...
    if (provider === "openai") {
      ...
      return (
        (tokens.prompt / 1_000_000) * costs.input + (tokens.completion / 1_000_000) * costs.output
      );
    }
  ...
  getCostSummary(): {
    ...
    return {
      ...
      perRequest:
        this.metrics.totalRequests > 0 ? this.metrics.totalCost / this.metrics.totalRequests : 0,
      perToken:
        this.metrics.totalTokens > 0 ? this.metrics.totalCost / this.metrics.totalTokens : 0,
      savings,
    };
  }
