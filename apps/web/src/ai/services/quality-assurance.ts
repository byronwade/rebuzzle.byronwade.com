...
export async function runQualityPipeline(
  ...
  if (qualityAnalysis.scores.overall >= 70 && (!adversarial || adversarial.passesAdversarialTest)) {
    verdict = "publish";
  } else if (
    finalScore >= 60 &&
    (!adversarial || adversarial.attacks.filter((a) => a.severity === "critical").length === 0)
  ) {
    verdict = "revise";
