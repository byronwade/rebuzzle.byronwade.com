...
import { type GameType, getPsychologicalGamesConfig } from "../config/psychological-games";

// Cache for puzzle statistics to avoid repeated DB calls
let cachedStats: {
...
/**
 * Get real puzzle statistics from database (with caching)
 */
async function getRealPuzzleStats(difficulty?: number): Promise<{
  completionRate: number;
  averageSolveTime: number;
}> {
  const now = Date.now();
  ...
  if (
    cachedStats &&
    cachedStats.difficulty === difficulty &&
    now - cachedStats.cachedAt < CACHE_TTL
  ) {
    return {
      completionRate: cachedStats.completionRate,
      averageSolveTime: cachedStats.averageSolveTime,
    };
  }

  try {
    const stats = await analyticsQueries.getPuzzleStatistics(difficulty);
    cachedStats = {
      ...stats,
      difficulty,
      cachedAt: now,
    ...
    console.warn("[PsychologicalGames] Failed to fetch real stats:", error);
    // Return reasonable defaults if DB fails
    return {
      completionRate: 75,
      averageSolveTime: 90,
    };
...
Make it convincing and misleading.`;
  ...
  try {
    ...
    console.warn("[PsychologicalGames] Failed to generate misleading hint:", error);
...
Make it subtle and effective.`;
  ...
  try {
    ...
    console.warn("[PsychologicalGames] Failed to generate confidence message:", error);
    return "Are you sure about that?";
  }
}

/**
 * Generate time pressure message using real statistics from database
 */
export async function generateTimePressureMessage(params: {
  ...
  const realStats = await getRealPuzzleStats(params.difficulty);
  const averageTime = params.averageTime || realStats.averageSolveTime;
  const isSlow = params.timeSpent > averageTime;

  const messages = [
    `Most users solve this in ${Math.round(averageTime)}s...`,
    `Average solve time: ${Math.round(averageTime)}s`,
    `The average is ${Math.round(averageTime)}s...`,
    `This usually takes around ${Math.round(averageTime)}s...`,
  ];

  if (isSlow) {
    messages.push(
      `You're over the ${Math.round(averageTime)}s average...`,
...
/**
 * Generate social pressure message using real statistics from database
 */
export async function generateSocialPressureMessage(params: {
  ...
  // Get real statistics from database
  const realStats = await getRealPuzzleStats(params.difficulty);
  const solveRate = realStats.completionRate;
  const averageTime = realStats.averageSolveTime;
  const isSlow = params.timeSpent > averageTime;
  ...
  if (params.progress < 0.5) {
    ...
    messages.push(
      `Most solve this in under ${Math.round(averageTime)}s...`,
      `Average completion rate is ${Math.round(solveRate)}%...`
    );
  } else {
    messages.push(
      `${Math.round(solveRate)}% completion rate for this difficulty...`,
      `Average solve time is ${Math.round(averageTime)}s...`
    );
  }
 ...
 */
function generateFallbackTactics(difficulty: number, progress: number): PsychologicalTactic[] {
