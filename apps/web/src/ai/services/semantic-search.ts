...
export async function recommendPuzzlesByUserHistory(
  ...
  const likedPuzzleIds = Array.from(new Set(likedPuzzles.map((a) => a.puzzleId))).slice(0, 5); // Use top 5 liked puzzles
  ...
  const recommendations = allPuzzles
    ...
    .filter(
      (item): item is { puzzle: WithId<Puzzle>; score: number; reason: string } => item !== null
    )
 ...
 */
async function updatePuzzleEmbedding(puzzleId: string, embedding: number[]): Promise<void> {
  const collection = getCollection<Puzzle>("puzzles");
