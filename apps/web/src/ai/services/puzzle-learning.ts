...
export async function analyzePuzzlePerformance(puzzleId: string): Promise<{
  ...
  const collection = await getCollection<PuzzleAttempt>("puzzleAttempts");
  const attempts = await collection.find({ puzzleId }).sort({ attemptedAt: -1 }).toArray();
  ...
  // Hints metrics
  const attemptsWithHints = attempts.filter((a) => a.hintsUsed !== undefined && a.hintsUsed > 0);
  ...
  const attemptsWithDifficulty = attempts.filter((a) => a.difficultyPerception !== undefined);
  const difficultyRating =
    attemptsWithDifficulty.length > 0
      ? attemptsWithDifficulty.reduce((sum, a) => sum + (a.difficultyPerception || 0), 0) /
        attemptsWithDifficulty.length
      : 0;

  // User satisfaction
  const attemptsWithSatisfaction = attempts.filter((a) => a.userSatisfaction !== undefined);
  const userSatisfaction =
    attemptsWithSatisfaction.length > 0
      ? attemptsWithSatisfaction.reduce((sum, a) => sum + (a.userSatisfaction || 0), 0) /
 ...
 */
export async function calculateActualDifficulty(puzzleId: string): Promise<number> {
 ...
 */
export async function generateImprovementSuggestions(puzzleId: string): Promise<{
  suggestions: Array<{
    category: string;
    suggestion: string;
