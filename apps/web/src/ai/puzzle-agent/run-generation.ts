...
function loadInstructions(): string {
  ...
  return [
    ...
    "- ANSWER-FIRST: call sample_answer_corpus / use brief seeds — pick the answer BEFORE designing the board. Do not freestyle a recycled trope.",
    "- Prefer MULTI-WORD answers (2–5 words) when difficulty ≥ 7 for uniqueness headroom.",
    "- ALWAYS call compose_puzzle_visual — unicode-only boards are rejected at publish.",
    "- Prefer ≥1 pictogram SVG; styled text (large/strike/stacked) is OK when typography/letter_play is the joke.",
    ...
    "- Invent ONE clean mechanism — new families OK: letter_play, container_phrase, direction_wordplay, hyphen_compound.",
    "- Avoid overused tropes (before, sunflower, piece of cake) unless the twist is genuinely new.",
    ...
    "- Hints: device family → structure → fragment → letter; never dump scaffolds before the final hint.",
    "- Pipeline must pass: validate → uniqueness → calibrate (in-band) → solvability → score_quality → check_anti_clone.",
...
function buildUserMessage(params: PuzzleGenerationParams, priorFailure?: string): string {
  ...
  return [
    ...
    "Workflow:",
    "1) get_puzzle_type_spec + get_difficulty_brief + get_generation_brief",
    "2) sample_answer_corpus (ANSWER-FIRST) + list_recent_answers — lock an unused answer",
    "3) list_technique_library → pick techniqueId that fits the answer shape, then compose_puzzle_visual + craft_hint_ladder",
    "4) validate → uniqueness → calibrate → stress_test_solvability → score_quality",
    "5) score_hint_fairness + score_shareability + check_anti_clone — revise if any fail",
    "6) critique_candidate + simulate_player_solve, then revise",
    "7) Return structured result with difficultyLevel + techniqueId + visual",
  ]
    .filter(Boolean)
    .join("\n");
}
