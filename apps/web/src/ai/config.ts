  ...
  switch (provider) {
    case "google":
      ...
      if (!process.env.GOOGLE_AI_API_KEY) {
        missing.push("GOOGLE_AI_API_KEY (or use AI_PROVIDER=gateway with only AI_GATEWAY_API_KEY)");
      }
      break;
    case "groq":
      ...
      if (!process.env.GROQ_API_KEY) {
        missing.push("GROQ_API_KEY (or use AI_PROVIDER=gateway with only AI_GATEWAY_API_KEY)");
      }
      break;
    case "xai":
      ...
      if (!process.env.XAI_API_KEY) {
        missing.push("XAI_API_KEY (or use AI_PROVIDER=gateway with only AI_GATEWAY_API_KEY)");
      }
      break;
    case "openai":
      ...
      if (!process.env.OPENAI_API_KEY) {
        missing.push("OPENAI_API_KEY (or use AI_PROVIDER=gateway with only AI_GATEWAY_API_KEY)");
      }
      break;
    case "gateway": {
