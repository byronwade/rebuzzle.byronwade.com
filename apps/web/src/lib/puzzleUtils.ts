...
export function getPuzzleDisplayFieldName(puzzleType: string): string {
  try {
    const _config = getPuzzleTypeConfig(puzzleType);
 ...
 */
export function getPuzzleDisplay(puzzle: PuzzleLike, puzzleType?: string): string {
  ...
  return (
    (puzzle.puzzleText as string) || (puzzle.content as string) || (puzzle.display as string) || ""
  );
}
