  ...
  switch (config.pattern) {
    case "invert-correct":
      ...
      for (let i = 0; i < modified.length; i++) {
        if (modified[i]?.status === "correct" && Math.random() < config.intensity) {
          modified[i] = {
            ...modified[i]!,
            status: "incorrect",
          };
    ...
    case "boost-incorrect":
      ...
      for (let i = 0; i < modified.length; i++) {
        if (modified[i]?.status === "incorrect" && Math.random() < config.intensity * 0.7) {
          modified[i] = {
            ...modified[i]!,
            status: "partial",
          };
    ...
    case "delay-correct":
      ...
      for (let i = 0; i < modified.length; i++) {
        if (modified[i]?.status === "correct" && Math.random() < config.intensity * 0.5) {
          modified[i] = {
            ...modified[i]!,
            status: "unknown",
          };
    ...
    case "false-close":
      ...
      for (let i = 0; i < modified.length; i++) {
        if (modified[i]?.status === "incorrect" && Math.random() < config.intensity * 0.6) {
          modified[i] = {
            ...modified[i]!,
            status: "partial",
          };
    ...
    case "confidence-drain":
      ...
      for (let i = 0; i < modified.length; i++) {
        if (modified[i]?.status === "correct" && Math.random() < config.intensity * 0.4) {
          modified[i] = {
            ...modified[i]!,
