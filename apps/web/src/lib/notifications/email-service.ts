...
export async function sendEmail(options: EmailOptions): Promise<EmailResult> {
  try {
    ...
    const fromEmail =
      process.env.RESEND_FROM_EMAIL || process.env.FROM_EMAIL || "notifications@byronwade.com";
...
export async function sendDailyPuzzleEmail(
  ...
  const unsubscribeUrl =
    options?.unsubscribeUrl || `${baseUrl}/unsubscribe?email=${encodeURIComponent(email)}`;
...
export async function sendNotificationWelcomeEmail(
  ...
  const { NotificationWelcomeEmail } = await import("../../../emails/notification-welcome");
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "https://byronwade.com";
...
export async function sendPasswordChangeEmail(
  ...
  const { PasswordChangeEmail } = await import("../../../emails/password-change");
  return sendEmailFromTemplate(
...
export async function sendAchievementUnlockedEmail(
  ...
  const { AchievementUnlockedEmail } = await import("../../../emails/achievement-unlocked");
  return sendEmailFromTemplate(
    React.createElement(AchievementUnlockedEmail, {
      ...
      achievementRarity: options.achievementRarity,
      achievementPoints: options.achievementPoints,
      achievementIcon: options.achievementIcon,
      totalUnlocked: options.totalUnlocked,
      totalAchievements: options.totalAchievements,
    }),
    email,
    `Achievement Unlocked: ${options.achievementName}!`
  );
}

/**
 * Send streak at risk notification email
 *
 ...
 * Only sent once per day, in the evening (around 8pm local time).
 */
export async function sendStreakAtRiskEmail(
  ...
  const { StreakAtRiskEmail } = await import("../../../emails/streak-at-risk");
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "https://byronwade.com";
  const puzzleUrl = options.puzzleUrl || baseUrl;
  const unsubscribeUrl =
    options.unsubscribeUrl || `${baseUrl}/unsubscribe?email=${encodeURIComponent(email)}`;

  return sendEmailFromTemplate(
    React.createElement(StreakAtRiskEmail, {
      username: options.username,
    ...
    email,
    `Your ${options.currentStreak}-day streak needs you!`
  );
}
