...
export function BlogPostPreview({ blogPost, onSave, onCancel }: BlogPostPreviewProps) {
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-2 text-muted-foreground text-sm">
                  Slug: <code className="rounded bg-muted px-2 py-1">{editedPost.slug}</code>
                </div>
              </div>
              <div className="rounded-lg border bg-muted/50 p-4">
                <div className="mb-2 font-medium text-sm">Excerpt</div>
                <div className="text-muted-foreground">{editedPost.excerpt}</div>
              </div>
              <div className="prose prose-lg dark:prose-invert max-w-none">
                <ReactMarkdown remarkPlugins={[remarkGfm]}>{editedPost.content}</ReactMarkdown>
              </div>
            </div>
          </TabsContent>
            ...
            <div className="space-y-4">
              ...
              {editedPost.publishedAt && (
                <div>
                  ...
                  <Input
                    ...
                    value={
                      editedPost.publishedAt
                        ? new Date(editedPost.publishedAt).toISOString().slice(0, 16)
                        : ""
                    }
                  />
                </div>
              ...
              <div>
                ...
                <div className="mt-2 space-y-1 text-muted-foreground text-sm">
                  ...
                  <div>Paragraphs: {editedPost.content.split(/\n\n+/).length}</div>
                </div>
              </div>
            </div>
          </TabsContent>
