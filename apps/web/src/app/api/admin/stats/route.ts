...
async function fetchAdminStats(dateStart: Date, dateEnd: Date) {
  try {
    ...
    const puzzleAttemptsCollection = getCollection<PuzzleAttempt>("puzzleAttempts");
    ...
    const [newUsersLast7Days, newPuzzlesLast7Days, newBlogPostsLast7Days, eventsLast7Days] =
      await Promise.all([
    ...
    const topUsers = await userStatsCollection.find({}).sort({ points: -1 }).limit(10).toArray();

    // Get user IDs for top users
    const topUserIds = topUsers.map((stats) => stats.userId);
    const topUsersData = await usersCollection.find({ id: { $in: topUserIds } }).toArray();
    ...
    const monthlyActiveUsers = await analyticsEventsCollection.distinct("userId", {
      timestamp: { $gte: dateStart, $lte: dateEnd },
      userId: { $exists: true, $ne: null as any },
    } as any);
    ...
    const avgSessionDuration =
      sessionsWithDuration.length > 0
        ? sessionsWithDuration.reduce((sum, session) => sum + (session.duration || 0), 0) /
    ...
    const returningUsers = allUserSessions.filter((s) => s.isReturningUser).length;
    ...
    const activeInLast7Days = await analyticsEventsCollection.distinct("userId", {
      timestamp: { $gte: sevenDaysAgo },
      userId: { $exists: true, $ne: null as any },
    });
    const totalUsersWithActivity = await analyticsEventsCollection.distinct("userId", {
      userId: { $exists: true, $ne: null as any },
    });
    const churnedUsers = totalUsersWithActivity.length - activeInLast7Days.length;
    const churnRate =
      totalUsersWithActivity.length > 0 ? (churnedUsers / totalUsersWithActivity.length) * 100 : 0;
    ...
    const usersWithFirstPuzzle = await puzzleAttemptsCollection.distinct("userId");
    ...
    const progressionFunnel = {
      ...
      conversionToFirstPuzzle:
        totalSignups > 0 ? (usersWithFirstPuzzle.length / totalSignups) * 100 : 0,
      conversionToRegular:
        usersWithFirstPuzzle.length > 0 ? (regularPlayers / usersWithFirstPuzzle.length) * 100 : 0,
    };
