  ...
  try {
    ...
    try {
      ...
      if (googleApiResponse.ok) {
        ...
        directApiTest = {
          success: true,
          availableModels: modelsData.models?.slice(0, 10).map((m: any) => m.name) || [],
          totalModels: modelsData.models?.length || 0,
        };
        console.log(`[AI Test] API key is valid! Found ${directApiTest.totalModels} models`);
      } else {
        ...
        console.error(`[AI Test] API key test failed: ${googleApiResponse.status}`, errorText);
      }
    } catch (error) {
      directApiTest = {
        success: false,
