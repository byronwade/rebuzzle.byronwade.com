...
import { ArchiveRestore, Gamepad2, Loader2, Send, ShieldCheck } from "lucide-react";
...
export function PuzzlePlaytestsClient() {
  return (
    <AuthGate>
    ...
    void (async () => {
      await withLoadingFlag(setLoading, async () => {
        ...
        try {
          ...
          if (cancelled) return;
          if (!response.ok || !data?.progress) fail(data?.error || "Failed to load playtest");
