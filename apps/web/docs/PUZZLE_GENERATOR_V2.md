...
Publication boards use a **viewport-invariant layout lock**: row breaks are planned once from the compact-320 profile (and grid columns from a fixed rule), then reused at mobile-375 and desktop-768 so cue adjacency cannot change with width. The React player consumes the same locked row membership instead of CSS `flex-wrap` reflow. High-coverage techniques receive a deterministic rule→graph composer (`simple_compound`, `obvious_emoji_sum`, `multi_emoji_compound`, `single_homophone`, `basic_positional`, `positional_phrase`, `spatial_preposition_play`, `math_symbol_wordplay`, `size_or_case_semantics`, `idiom_as_picture`, `false_lead_visual`) that chooses layout and joiners/emphasis before invent spend. For answer-seed runs the host preflight board is **structurally authoritative**: invent may fill catalog assets on matching layers but cannot rewrite layout or cue topology. A near-miss / alternate-answer stress gate rejects answers that collide with close phrase-bank cousins and fails closed when blind-solve wrong parses are near-miss cousins of the intended answer. Apex finalist pre-ranking blends tournament rubric scores with an Elo-style prior derived from live technique solve rates so expensive rendered evaluation prefers difficulty-fit drafts.
