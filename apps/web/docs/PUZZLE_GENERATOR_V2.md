...
Publication boards use a **viewport-invariant layout lock**: row breaks are planned once from the compact-320 profile (and grid columns from a fixed rule), then reused at mobile-375 and desktop-768 so cue adjacency cannot change with width. The React player consumes the same locked row membership instead of CSS `flex-wrap` reflow. High-coverage techniques receive a deterministic rule→graph composer (`simple_compound`, `obvious_emoji_sum`, `single_homophone`, `basic_positional`, `positional_phrase`, `spatial_preposition_play`, `math_symbol_wordplay`, `size_or_case_semantics`) that chooses layout and joiners/emphasis before invent spend. For answer-seed runs the host preflight board is **structurally authoritative**: invent may fill catalog assets on matching layers but cannot rewrite layout or cue topology. A near-miss / alternate-answer stress gate rejects answers that collide with close phrase-bank cousins.
