...
Publication boards use a **viewport-invariant layout lock**: row breaks are planned once from the compact-320 profile (and grid columns from a fixed rule), then reused at mobile-375 and desktop-768 so cue adjacency cannot change with width. The React player consumes the same locked row membership instead of CSS `flex-wrap` reflow. All seventeen named techniques receive a deterministic rule→graph composer that chooses layout and joiners/emphasis before invent spend. For answer-seed runs the host preflight board is **structurally authoritative**: invent may fill catalog assets on matching layers but cannot rewrite layout or cue topology. A near-miss / alternate-answer stress gate rejects answers that collide with close phrase-bank cousins and fails closed when blind-solve wrong parses are near-miss cousins of the intended answer. Blind and progressive-hint screenshot solves both use a DescProg (DESCRIPTION → RELATIONS → PROGRAM → HYPOTHESES) ladder. Apex finalist pre-ranking blends tournament rubric scores with Elo priors: soft technique solve-rate priors plus persisted pairwise answer Elo in `puzzleEloRatings` (updated from human playtest strata).
...
Hybrid image tiles obey the same semantic contract as pictograms. Every new image layer declares one concrete recognition concept, stores an immutable embedded image rather than a changeable URL, and contributes that concept to blind object votes, multiplicity, novelty fingerprints, and live-canary evidence. A visually attractive scene cannot pass because its alt text says `car`; two independent judges must actually identify the rendered tile as a car in every production profile. Legacy image layers without an explicit concept fail closed.
...
Playtest queue failures are logged but do not retroactively break a daily publication that already passed all automated gates and database locks. Playtest technique strata now feed curriculum technique bias (merged with live `metadata.liveSolveRate`), pairwise Elo updates in `puzzleEloRatings`, and technique-drift watch reasons on `human-calibration-gate-v1`. Release-cohort failures still force reserve; collecting samples stay watch/insufficient. No human playtest decisions have been fabricated.

The playtest admin page can also audit historical records before mutating the queue. The dry run scans a bounded newest-first window and reports eligible, already queued, excluded, and truncated counts. Apply must be explicit and is capped at 200 candidates per request. Historical rows qualify only when they are AI-generated rebuses with a valid composed visual, exact rendered board-recognition evidence, all three responsive blind profiles, all three accepted editorial profiles, `structural-v1` novelty evidence, and an automated solve-rate estimate. Reserve inventory, lab previews, non-rebuses, legacy Unicode boards, and partially gated generations are excluded so old weak pipelines cannot contaminate the current-generator calibration cohort. Candidate uniqueness makes repeated audits and applies idempotent.

Backfill eligibility is fail-closed at the evidence shape, not just at the row count: the blind record must contain exactly `compact-320`, `mobile-375`, and `desktop-768`, each with the required independent judge quorum and target/top/dominant answer agreement. Editorial profile coverage must also be exact. This prevents malformed or partial historical telemetry from becoming human-calibration evidence.
...
- [Do You See Me](https://aclanthology.org/2026.eacl-long.343/) reports a large human-model gap on controlled visual-perception tasks and warns that multimodal models can exploit answer-format shortcuts, supporting open-ended cue inventories instead of multiple-choice checks.
- [VL-RewardBench](https://openaccess.thecvf.com/content/CVPR2025/html/Li_VL-RewardBench_A_Challenging_Benchmark_for_Vision-Language_Generative_Reward_Models_CVPR_2025_paper.html) finds that vision-language judges fail predominantly on basic perception, supporting separate per-cue perception evidence before whole-board reasoning scores are trusted.
