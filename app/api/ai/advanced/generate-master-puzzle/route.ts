...
import { generateMasterPuzzle, generateMasterBatch } from "@/ai/advanced"
    ...
    if (mode === "batch") {
      ...
      return NextResponse.json({
        ...
        stats: {
          ...
          avgQuality: results.reduce((sum, r) => sum + r.metadata.qualityMetrics.scores.overall, 0) / results.length,
          avgUniqueness: results.reduce((sum, r) => sum + r.metadata.uniquenessScore, 0) / results.length,
...
export async function GET() {
  return NextResponse.json({
    ...
    description: "Generates ultra-high-quality, unique, challenging rebus puzzles",
    features: [
